...
class MatrixStatusUpdater:
    ...
    async def update(
        ...
        async with self.lock:
            ...
            log_res = None
            if is_new or is_exit or force:
                if not self.enabled:
                    log_res = (None, f"[{self.username}] (Disabled)", True)
                else:
                    activity_str = "Offline" if is_exit else activity
                    user_str = f"[{self.username}] [{self.current_presence}]"
                    log_res = (activity_str, user_str, False)
    ...
    while True:
        try:
            ...
            for updater in updaters:
                log_res = await updater.update(activity, title=title)
                if log_res:
                    act_str, usr_str, is_disabled = log_res
                    if is_disabled:
                        disabled_users.append(usr_str)
                    else:
                        updates_by_status.setdefault(act_str, []).append(usr_str)

            for act_str, users in updates_by_status.items():
                print(f"Matrix Status -> {act_str}", flush=True)
                for user in users:
